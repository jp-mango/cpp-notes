...
int main() {
  ...
  cout << "The last vowel is: " << vowels[4]
       << endl; // anything larger than 4 will result in an out of bounds error.
  ...
  vector<double> temps_vector(
      30, 80.5); // will create a vector of length = 30, with each value as 80.5
  cout << "First vector temp: " << temps_vector[0] << endl;
  cout << "Last vector temp: " << temps_vector[29]
       << endl; // using a subscript operator,[], w/ a vector will not out of
                // bounds check
  vector<double> double_vector = {2.5, 5.7, 8.2, 3.0, 9.9};
  cout << "Last vector value: " << double_vector.at(double_vector.size() - 1)
       << endl; // .size() returns the length of a vector, subtract 1 to avoid
                // out of bounds
  double_vector.push_back(12.7); // adds 12.7 to the end of the the vector
  cout << "Now the last vector value is: "
       << double_vector.at(double_vector.size() - 1) << endl;
  //---------------------------------------------------------------------------------------------------------
  return 0;
}